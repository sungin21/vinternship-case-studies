...
for (
    ...
    if (tx.type === "priority") {
        ...
        console.log(
            "Priority transaction encountered, stopping."
        );
...
console.log(
    `Processed in while loop: ${processedWhile}`
);


// 3. do...while Loop
let pendingReturns: number = 0;
...
do {
    ...
    if (tx.type === "return") {
        ...
        console.log(
            `Handling return transaction ${tx.id}`
        );
...
let priorityFound: boolean = false;
...
do {
    ...
    console.log(
        `Dynamic return processed: ${tx.id}`
    );
